...
public:
	...
	enum Duration
	{
		...
		// For constructing the most _negative_ Time that can be represented.
		MIN,

		// For constructing the most positive Time that can be represented.
		MAX,
...
//---------------------------------------------------------------------------
Time::Time() : privRawTime(0)
{
}


Time::Time(const Time& rhs) : privRawTime(rhs.privRawTime)
{
}
