...
extern std::vector<FBX_Trilist> triList;
        ...
        for (int j = 0; j < pMesh->GetElementNormalCount(); j++)
        {
			...
			if (leNormals->GetMappingMode() == FbxGeometryElement::eByControlPoint)
			{
				...
				if (leNormals->GetReferenceMode() == FbxGeometryElement::eDirect)
				{
					...
					temp.x = (float)lControlPoints[i][0];
					temp.y = (float)lControlPoints[i][1];
					temp.z = (float)lControlPoints[i][2];
					temp.nx = (float)(leNormals->GetDirectArray().GetAt(i)[0]);
					temp.ny = (float)(leNormals->GetDirectArray().GetAt(i)[1]);
					temp.nz = (float)(leNormals->GetDirectArray().GetAt(i)[2]);
					skinverts.push_back(temp);
				}
...
// Keenan(7) 
    ...
    for (i = 0; i < lPolygonCount; i++)
    {
		...
		for (j = 0; j < lPolygonSize; j++)
		{
			...
			if (j == 0)
			{
				tmpTrilist.v0 = lControlPointIndex;
			}
			else if (j == 1)
			{
				tmpTrilist.v1 = lControlPointIndex;
			}
			else if (j == 2)
			{
				tmpTrilist.v2 = lControlPointIndex;
			}
			else
			{
				assert(false);
			...
			for( l = 0; l < pMesh->GetElementNormalCount(); ++l)
			{
				...
				if(leNormal->GetMappingMode() == FbxGeometryElement::eByPolygonVertex)
				{
					...
					skinverts[lControlPointIndex].nx = (float)pNormal.mData[0];
					skinverts[lControlPointIndex].ny = (float)pNormal.mData[1];
					skinverts[lControlPointIndex].nz = (float)pNormal.mData[2];
